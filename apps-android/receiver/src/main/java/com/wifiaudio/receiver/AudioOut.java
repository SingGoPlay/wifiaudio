...
public interface AudioOut {
    ...
    /** 写入 PCM 字节流（little-endian 16bit），默认转 short 后走 writePcm；AudioTrack 覆盖为直写 */
    default void writePcmBytes(byte[] data, int len) {
        int samples = len / 2;
        short[] s = new short[samples];
        java.nio.ByteBuffer.wrap(data, 0, len).order(java.nio.ByteOrder.LITTLE_ENDIAN).asShortBuffer().get(s, 0, samples);
        writePcm(s, samples);
    }
    ...
    final class AudioTrackOut implements AudioOut {
        ...
        @Override
        public void writePcmBytes(byte[] data, int len) {
    ...
    final class AaudioOut implements AudioOut {
        ...
        @Override
        public void writePcmBytes(byte[] data, int len) {
            // 注意：必须传"帧数"（= 总采样数 / 声道数），否则 AAudio 写 2 倍帧数导致数据错乱/电音
            if (handle == 0 || channels <= 0) return;
            int samples = len / 2;
            short[] s = new short[samples];
            java.nio.ByteBuffer.wrap(data, 0, len)
                    .order(java.nio.ByteOrder.LITTLE_ENDIAN)
                    .asShortBuffer().get(s, 0, samples);
            AaudioNative.write(handle, s, samples / channels);
        }
